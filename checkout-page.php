...
for($i=0;$i<100;$i++)  {
	...
	if(isset($_POST["price"][$i])){
		...
		if(!isset($_SESSION['email'])){
			...
			// this will help me to go to category page after login page
			// $_SESSION['category_name'] = $_POST["category_name"][$i];
		}
		else{
			$user_email = $_SESSION['email'];
			$product_name[$i] = $_POST["title"][$i];
			$product_category[$i] = $_POST["category_name"][$i];
			$product_price[$i] = $_POST["price"][$i];
			$product_quantity = 1;
			$new_price = round($product_price[$i] * $product_quantity, 2);
			...
			if ($alreadyInCartNum>0) {
			 	echo "<script>
						...
						window.location.href = 'category-filter.php?category_name=$product_category[$i]#$product_name[$i]';
						// window.history.back();
					</script>
				";
			}
	...
	if(isset($_POST["price2"][$i])){
		...
		if(!isset($_SESSION['email'])){
			...
			$product_name[$i] = $_POST["title"][$i];
			$product_category[$i] = $_POST["category_name"][$i];
			$product_price[$i] = $_POST["price2"][$i];
			$product_quantity = 1;
			$new_price = round($product_price[$i] * $product_quantity, 2);
			$order_status = "In Cart";
			...
			if ($alreadyInCartNum>0) {
			 	echo "<script>
						window.alert('Item already in Cart');
						window.location.href = 'search-results.php?keyword=$keyword#$product_name[$i]';
					</script>
				";
			}
			else{
